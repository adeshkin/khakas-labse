...
Почему именно так — TAPT (шаг 3) не конфликтует с задачей выравнивания (шаг 4): сначала модель учит "грамматику" языка, потом — соответствие смыслов. NSP-задача сознательно не используется (современные модели вроде RoBERTa/XLM-R показывают, что она не нужна при наличии качественного MLM).

Рекомендуемые эпохи/LR для датасета ~150к пар + ~100к моно:
...
## Токены и конфиденциальные данные

Проверено: в git-истории репозитория утечек токенов/паролей не обнаружено.

В корне лежит `.env` с реальными токенами HuggingFace (`READ_HF_TOKEN`, `WRITE_HF_TOKEN`). Файл в `.gitignore`, в репозиторий не попадает — но хранится на диске в открытом виде. При необходимости обновить/отозвать: https://huggingface.co/settings/tokens

`experiments/finetune_labse_lingtrain.py` использует Google Colab `userdata` для получения токена — тоже не хардкод.
